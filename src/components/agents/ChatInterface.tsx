...
export default function ChatInterface() {
    ...
    return (
        <div className="flex flex-col h-full min-h-0">
                                            ...
                                            (msg.content.includes('## 1. 📝 Blog Post'))
                                                ...
                                                <button
                                                    onClick={async () => {
                                                        ...
                                                        // 현재 페이지에서 직접 postMessage → Hwack 확장 프로그램이 수신
                                                        try {
                                                            window.postMessage({
                                                                type: 'HWACK_UPLOAD_NAVER',
                                                                data: postData
                                                            }, '*');
                                                            showToast('네이버 블로그로 전달 완료');
                                                        } catch (e) {
                                                            console.error('Handoff error:', e);
                                                            showToast('전달 실패. 다시 시도해주세요.', 'error');
                                                        }
                                                    }}
                                            ...
                                            (msg.content.includes('## 2. 🎨 Instagram Content'))
                                                ...
                                                <button
                                                    onClick={async () => {
                                                        ...
                                                        // 직접 postMessage → Hwack 확장 프로그램 수신
                                                        try {
                                                            window.postMessage({
                                                                type: 'HWACK_UPLOAD_INSTA',
                                                                data: { caption: cleanCaption, blocks: blocks }
                                                            }, '*');
                                                            showToast('인스타그램으로 전달 완료');
                                                        } catch (e) {
                                                            console.error('Handoff error:', e);
                                                            showToast('전달 실패. 다시 시도해주세요.', 'error');
                                                        }
                                            ...
                                            (msg.content.includes('## 3. 🥕 Danggeun'))
                                                ...
                                                <button
                                                    onClick={async () => {
                                                        ...
                                                        try {
                                                            window.postMessage({
                                                                type: 'HWACK_UPLOAD_DANG',
                                                                data: { title: title, content: lines.slice(bodyStartIndex).join('\n') }
                                                            }, '*');
                                                            showToast('당근마켓으로 전달 완료');
                                                        } catch (e) {
                                                            console.error('Handoff error:', e);
                                                            showToast('전달 실패. 다시 시도해주세요.', 'error');
                                                        }
            ...
            {/* Toast 알림 */}
            {toast && (
                <div className={`fixed bottom-6 right-6 z-50 flex items-center gap-2 px-4 py-3 rounded-xl shadow-lg text-sm font-medium transition-all animate-fade-in
                    ${toast.type === 'success' ? 'bg-secondary text-primary' : 'bg-red-500 text-white'}`}>
